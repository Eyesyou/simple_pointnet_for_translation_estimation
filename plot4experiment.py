    ...
    for i in range(n):
        ...
        if point_clouds is not None:
            mlab.points3d(point_clouds[i, :, 0], point_clouds[i, :, 1], point_clouds[i, :, 2],
                          ...
                          scale_factor=0.005, figure=mfig)
        else:
...
def vis_first_layer(net_input_point_cloud, first_layrer_output, vis_rate=1/10):
    """
    input to the network should be cubic grid point cloud
    :param net_input_point_cloud: 1024 x 3
    :param first_layrer_output:  1 x 1024 x 64   number of points x features
    :return:
    """
    ...
    for i in range(np.shape(first_layrer_output)[1]):
        ...
        mlab.points3d(net_input_point_cloud[idx, 0], net_input_point_cloud[idx, 1], net_input_point_cloud[idx, 2],
